...
export default function RoundsPage() {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-start px-3 sm:px-4 py-5 sm:py-8 md:py-10 max-w-6xl mx-auto w-full">
      ...
      <div className="text-center space-y-1.5 sm:space-y-2 mb-6 sm:mb-8">
        <div className="inline-flex items-center gap-2 px-3 sm:px-3.5 py-1 rounded-full bg-[#240c0a]/80 border border-[#ff9100]/40 text-[10px] sm:text-xs font-mono font-bold text-[#ff9100] shadow-md">
        ...
        <p className="text-xs sm:text-sm text-[#fff1d1]/70 max-w-xl mx-auto leading-relaxed">
      ...
      <AnimatePresence>
        {tooltipText && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
            className="mb-5 sm:mb-6 px-4 py-2 rounded-xl bg-[#df301c]/20 border border-[#df301c]/50 text-[#f04835] text-xs sm:text-sm font-semibold flex items-center gap-2 shadow-lg"
          >
            <Lock className="w-4 h-4 text-[#df301c] shrink-0" />
      ...
      {/* 3 Large Round Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 w-full mb-8 sm:mb-10">
        {roundsConfig.map((round, idx) => {
          ...
          return (
            <motion.div
              ...
              className={`relative rounded-2xl sm:rounded-3xl p-5 sm:p-7 flex flex-col justify-between transition-all duration-300 cursor-pointer overflow-hidden border bg-gradient-to-b ${
                round.isUnlocked
            ...
            >
              ...
              <div className="flex items-center justify-between gap-2 mb-3.5 sm:mb-4 relative z-10">
                <span
                  className={`text-[11px] sm:text-xs font-mono font-bold px-2.5 py-1 rounded-lg border ${
                    round.isUnlocked
                ...
                {round.isDone ? (
                  <span className="flex items-center gap-1.5 text-[11px] sm:text-xs font-bold text-[#00b7cd] bg-[#00b7cd]/20 border border-[#00b7cd]/40 px-2.5 py-1 rounded-full">
                    <CheckCircle2 className="w-3.5 h-3.5 text-[#00b7cd] shrink-0" />
                    <span>Done</span>
                  </span>
                ) : round.isUnlocked ? (
                  <span className="flex items-center gap-1 text-[11px] sm:text-xs font-bold text-[#ff9100] bg-[#ff9100]/20 border border-[#ff9100]/40 px-2.5 py-1 rounded-full">
                    <Unlock className="w-3.5 h-3.5 text-[#ff9100] shrink-0" />
                    <span>Active</span>
                  </span>
                ) : (
                  <span className="flex items-center gap-1 text-[11px] sm:text-xs font-bold text-[#df301c] bg-[#240c0a]/80 border border-[#df301c]/40 px-2.5 py-1 rounded-full">
                    <Lock className="w-3.5 h-3.5 text-[#df301c] shrink-0" />
                    <span>Locked</span>
                  </span>
                )}
              </div>
              ...
              <div className="space-y-2.5 sm:space-y-3 my-2 relative z-10">
                <div
                  className={`w-12 h-12 sm:w-14 sm:h-14 rounded-2xl flex items-center justify-center border shadow-inner ${
                    round.isUnlocked
                      ? round.iconBg
                ...
                <div>
                  <span className="text-[11px] sm:text-xs font-mono font-bold uppercase tracking-wider text-[#ff9100]">
                    Round {round.num}
                  </span>
              ...
              {/* Bottom: Score & Action */}
              <div className="pt-4 sm:pt-6 mt-3 sm:mt-4 border-t border-white/10 relative z-10 flex items-center justify-between">
                <div>
                  <div className="text-[10px] sm:text-[11px] font-mono text-[#fff1d1]">
                ...
                <div
                  className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center transition-transform shrink-0 ${
                    round.isUnlocked
                ...
                >
                  {round.isUnlocked ? (
                    <ArrowRight className="w-4 h-4 sm:w-5 sm:h-5 text-[#ff9100]" />
                  ) : (
      ...
      {allCompleted ? (
        <motion.div
          ...
          className="w-full bg-gradient-to-r from-[#381e05]/90 via-[#240c0a]/90 to-[#0a282c]/90 rounded-2xl sm:rounded-3xl p-5 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-4 border border-[#ff9100]/40 shadow-2xl"
        >
          <div className="flex flex-col sm:flex-row items-center gap-3.5 sm:gap-4 text-center sm:text-left">
            <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-[#ff9100]/20 border border-[#ff9100]/50 flex items-center justify-center text-[#ff9100] shrink-0">
            ...
            <div>
              ...
              <p className="text-xs sm:text-sm text-[#fff1d1]">
                Total Score: <strong className="text-[#00b7cd] font-mono text-sm sm:text-base">{student.totalScore} Points</strong>
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            <button
              ...
              className="w-full sm:w-auto px-6 py-3.5 rounded-xl bg-gradient-to-r from-[#ff9100] via-[#ffa726] to-[#fff1d1] hover:from-[#ffa726] hover:to-[#fff8e6] text-[#160706] font-black text-xs sm:text-sm transition-all shadow-[0_0_20px_rgba(255,145,0,0.4)] flex items-center justify-center gap-2 cursor-pointer"
            >
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-2.5 w-full max-w-4xl text-xs text-[#00b7cd]/80 pt-4 border-t border-white/10 text-center sm:text-left">
          ...
          <button
            ...
            className="hover:text-[#ff9100] flex items-center justify-center gap-1 transition-colors cursor-pointer py-1 px-2 rounded-lg bg-white/5 sm:bg-transparent"
          >
